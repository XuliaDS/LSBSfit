		...
		int deg, double *rms, double LS_tol, int maxIter)
{
	...
	while ( (L2error > LS_tol) && (iter < maxIter) && (n < m) ){
		...
		if ( stat != EGADS_SUCCESS ) {
			...
			redundantKnotSeq[redundantKnots] = newKnot;
			++redundantKnots;
			printf(" L2 error %11.4e  prev %11.4e update %11.4e\n",L2error, prevL2error, updateTOL);
						...
						if ( (i < spanLocation - header[1])  || (i > spanLocation + 2 ) ){
							if (badCount > 0 ) {
								for ( j = 0 ; j < badCount; ++j) {
									if (fabs(bSpline[badKnotSeq[j]] - bSpline[i] ) < DEPS ) {
										j = MAX_KNOTS-1;
									}
								}
								if ( j != MAX_KNOTS) {
		...
		if ( update_step == 1 ) {
			...
			++accepted;
			updateTOL      = MAX(L2error*0.5,0.1*LS_tol);
			//updateTOL    = 0.1*L2error;//0.0;//L2error*0.1;
			printf(" LL2/prev  %11.4e   prev/L2   %11.4e \n",log10(L2error/prevL2error),log10(prevL2error/L2error) );
#ifdef SAVE_ITER_NORMS
			fprintf(fn, "%d\t%11.5e\t%11.5e\t%11.5e\t%11.5e\t%11.5e\n",iter,L2error, prevL2error, updateTOL,fabs(log(L2error/prevL2error)), L2error*fabs(log(prevL2error/L2error)));
#endif
			bestL2error    = L2_INIT;
			spanLocation   = -1;
			badCount       = 0 ;

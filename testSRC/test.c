    ...
    int *spanLocation,  double  *newKnot, double *inL2error, double updateTOL)
{
  ...
  int     i,j, bestSpanLocation = -1,  tPos, v, knotCount ;
    ...
    int deg, double *rms, double LS_tol, int maxIter, double delta)
{
  ...
  int    rejections, accepted, spanLocation,  badCount, redundantKnots, update_step;
  ...
  while ( (L2error > LS_tol) && (iter < maxIter) && (n < m) ){
    ...
    char buffer[34];
    snprintf(buffer, sizeof(char) * 32, "error%i.txt", iter);
    Linf = 0.0;
    FILE *fIn = fopen(buffer, "w");
    for ( i =0 ; i < m ;++i) {
      fprintf(fIn,"%d\t%lf\t%.15lf\n", i,t[i], error[i]);
      Linf = MAX(Linf,error[i]);
    }
    fprintf(fIn,"#%11.5e    %11.5e\n",L2error,Linf);
    fclose(fIn);
    if ( stat != EGADS_SUCCESS ) {
    ...
    if ( update_step == 1 ) {
      if (falseIter > 0) {
        stat = get_minimal_knots(backHeader, backSpline, m, error, t, XYZ, redundantKnots, redundantKnotSeq, header, &bSpline, &L2error);
        if (stat != EGADS_SUCCESS) {
          printf(" stat GET_MINIMAL_KNOTS %d!!\n",stat);
          goto cleanup;
        }
        if ( (fabs(L2error - prevL2error )  < delta ) &&  (falseIter == MAX_FALSE_ITERS ))
        {
          for ( i = 0 ; i < 4; ++i) header[i] = backHeader[i];
          ...
          L2error = backL2error;
          printf(" I have done %d iterations  and L2 %11.4e ~ prev L2 %11.4e . Leave Fitting with 'best' approximation %d\n",falseIter,L2error, prevL2error, stat);
          stat = EGADS_SUCCESS;
          break;
        }
        else if (fabs(L2error - prevL2error )  >= updateTOL) falseIter = 0;
      }
      else falseIter      = 0 ;
      ++accepted;
      spanLocation   = -1;
      ...
      updateTOL      = MAX(L2error*log10(prevL2error/L2error),LS_tol*0.1) ;
    }
    if ( falseIter == MAX_FALSE_ITERS ) {
      ...
      for ( i = 0 ; i < 3*backHeader[2]+ backHeader[3]; ++ i)	bSpline[i] = backSpline[i];
      L2error = backL2error;
      printf(" I have done %d iterations with no improvement. Leave Fitting with 'best' approximation %d\n",falseIter,stat);
      stat = EGADS_SUCCESS;
      break;
    }
  }
  printf(" Leaving Solver with norm %11.4e < %11.4e\n TOTALS: accepted %d rejected %d \n",L2error, LS_tol, accepted, rejections);
#if SAVE_ITER_NORMS
  fclose(fn);
#endif
  if (stat != EGADS_SUCCESS) goto cleanup;

...
getParameterVector(double *data, int dimData, double *t, int paramType)
{
  ...
  for (k = 1; k < dimData - 1; k++) {
    ...
    t[k] = t[k-1] + pow(arc,exp_type)/diam;
  }
  return EGADS_SUCCESS;
}
...
static int
get_max_weighted_span(int offset, // = degree (start looking only at the interior knots)
    ...
    double *tVal)     // stores the t with maximum error at the maximum span
{
...
#else
          knotErr[i] = MAX(error[j],knotErr[i]);
#endif
          if (first_t == -1) {
            ids[i]  = j;
            first_t = 0;
  ...
  *errId = -1;
  ...
  if (*errId != -1) {
    first_t = ids[*errId]+1;
    errMax  = error[first_t];
    for ( i = 2 ; i < knotPool[*errId]; ++i ) {
      if ( error[ids[*errId] + i ] > errMax) {
        first_t = ids[*errId] + i ;
        errMax  = error[first_t];
      }
    }
    *tVal = t[first_t];
    if      ( fabs(*tVal - bSpline[*errId + offset])    < MIN_KNOT_DIST) *tVal = bSpline[*errId + offset ]   + MIN_KNOT_DIST;
    else if ( fabs(*tVal - bSpline[*errId + offset +1]) < MIN_KNOT_DIST) *tVal = bSpline[*errId + offset +1] - MIN_KNOT_DIST;
    stat    = EGADS_SUCCESS;
  }
#endif
#ifdef KNOT_WEIGHTED  
    ...
    int *spanLocation,  double  *newKnot, double *inL2error, double updateTOL)
{
  ...
  bestSpanLocation = -1;
  do {
    stat = get_max_weighted_span(deg, nSpans, badSpanVec, &locBadSpans, bSpline, error, t,  m, &tPos, &tVal);
    if (stat == EGADS_SUCCESS) {
    ...
    int deg, double *rms, double LS_tol, int maxIter)
{
  ...
  paramType = 1; // 1 = chord length, otherwise it uses centripetal
  nKnots    = n + deg + 1;
  header[0] = 0 ;
  header[1] = deg;
  header[2] = n;
...
#endif
#ifdef PRINT_SCALED_DATA
  fn = fopen("scaled_data","w");
  if (fn == NULL ) {
    stat = EGADS_MALLOC;
    goto cleanup;
  }
  for (k = 0; k < m; k++) fprintf(fn,"%lf\t%lft%lf\n",XYZ[3*k], XYZ[3*k+1], XYZ[3*k+2]);
  fclose(fn);
#endif
  ...
  while ( (L2error > LS_tol) && (iter < maxIter) && (n < m) ){
      ...
      if ( (L2error > prevL2error)  || ( fabs(prevL2error - L2error) < updateTOL ) ){
        ...
        if ( falseIter == 0 ) {
          for ( i = 0 ; i < header[3]; ++i) {
            if ( (i < spanLocation - header[1])  || (i >= spanLocation + 1 ) ){
              j = 0;
              if (badCount > 0 ) {
                for ( j = 0 ; j < badCount; ++j) {
                  if (fabs(bSpline[badKnotSeq[j]] - bSpline[i] ) < DEPS ) j = MAX_KNOTS;
                }
              }
              if ( j < MAX_KNOTS) {
          ...
          badKnotSeq[badCount] = spanLocation;
          ++badCount;
        }
    ...
    if ( update_step == 1 ) {
      if ( falseIter > 0) {
        double auxL2 = MAX(prevL2error,L2error);
        stat    = get_minimal_knots(backHeader, backSpline, m, error, t, XYZ, redundantKnots, redundantKnotSeq, header, &bSpline, &auxL2, &reductions);
        L2error = auxL2;
        if (stat != EGADS_SUCCESS) {
      ...
      else if ( falseIter > MAX_FALSE_ITERS) {
#ifdef PRINT_MESSAGES
        printf(" Cannot refine more the Bspline. Leave Fitting with 'best' approximation %d\n",stat);
#endif
        for ( i = 0 ; i < 4; ++i) header[i] = backHeader[i];
      ...
      updateTOL = 0.0;//(L2error*0.01, LS_tol);
      //updateTOL = MAX(log10(prevL2error/L2error)*L2error,LS_tol);
#ifdef SAVE_ITER_NORMS
      fprintf(fn, "%d\t%11.5e\t%11.5e\t%11.5e\t%11.5e\t%11.5e\n",iter,L2error, prevL2error, updateTOL,fabs(log(prevL2error/L2error)), L2error*fabs(log(prevL2error/L2error)));
#endif
    }
  ...
  *rms   = L2error;
  ...
  for ( i = 0; i < m; ++i) {
    ...
    if (stat != EGADS_SUCCESS) {
      ...
      fprintf(fbSpline,"%.11f\t%.11f\t%.11f\t%d\t%.11f\n",point[0],point[1],point[2],i,t[i]);
  }

		...
		int deg, double *rms, double LS_tol, int stepControl, int maxIter, double delta)
{
	...
	int    falseIter, falseIterAcc, headerCopy[4], header[4], backHeader[4], badKnotSeq[MAX_KNOTS];
	...
	while ( (L2error > LS_tol) && (iter < maxIter) && (n < m) ){
		...
		if ( (L2error > prevL2error)  || ( fabs(prevL2error - L2error) < updateTOL ) ){
			...
			++falseIter;
			update_step = 0;
			if ( (falseIter >= MAX_FALSE_ITERS) && (prevL2error > L2error) ) {
				update_step = 1;
			}
			else {
				for ( i = 0 ; i < header[3]; ++i) {
		...
		if ( falseIter == MAX_FALSE_ITERS ) {
			...
			stat = EGADS_SUCCESS;
			++falseIterAcc;
			if (falseIterAcc == MAX_FALSE_ITERS)	break;
			else updateTOL = 0.0;
		}
	}
